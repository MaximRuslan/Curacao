...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <div class="btn-group pull-right m-b-20">
                <a data-toggle="modal" href="#userStatusModal" class="btn btn-default waves-effect waves-light">Add</a>
            </div>
    ...
    @include('common.delete_confirm',[
        'modalId'=>'deleteStatus',
        'action'=>route('user-status.destroy','deleteId'),
        'item'=>'it',
        'callback'=>'showMsg'
        ])

    @include('admin.userstatus.create')
@endsection
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#userStatusModal').on('hidden.bs.modal', function () {
                $('#userStatusModal').find('form')[0].reset();
                $('#userStatusModal').find('form').find('input[name="id"]').val('');
                $('#userStatusModal').find('input').removeAttr('disabled');
                $('#userStatusModal').find('button[type="submit"]').show();
            })
        });

        function setEdit(id, type='') {
            var action = '{{route('user-status.show','')}}/' + id
            $.ajax({
                type: 'GET',
                url: action,
                data: {},
                ...
                success: function (data) {
                    setFormValues('userStatus_form', data.inputs);
                    $('#userStatusModal').modal('show');
                    if (type == 'view') {
                        setTimeout(function () {
                            $('#userStatusModal').find('input').attr('disabled', 'disabled');
                            $('#userStatusModal').find('button[type="submit"]').hide();
                        }, 100);
                    }
                },
