...
<div class="row">
	<div class="col-sm-12">
		<div class="btn-group pull-right m-b-20">
			<a data-toggle="modal" href="#loanStatusModal" class="btn btn-default waves-effect waves-light">Add</a>
		</div>
...
<div class="row">
	<div class="col-12">
		<div class="card-box table-responsive">
			<table id="loan-status-table" class="table  table-striped table-bordered" style="width:100%">
				<thead>
					<tr>
						<th>Title ENG</th>
						<th>Title ESP</th>
...
@include('common.delete_confirm',[
	'modalId'=>'deleteLoanStatus',
	'action'=>route('loan-status.destroy','deleteId'),
	'item'=>'it',
	'callback'=>'showMsg'
	])

	@include('admin.loanstatus.create')
@endsection
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			oTable = $('#loan-status-table').DataTable({
				processing: true,
				serverSide: true,
				ajax: {
					"url": "{{route('loan-status-data')}}",
					"type": "POST"
				},
				order: [],
			...
			$('#loanStatusModal').on('hidden.bs.modal', function () {
				$('#loanStatusModal').find('form')[0].reset();
				$('#loanStatusModal').find('form').find('input[name="id"]').val('');
				$('#loanStatusModal').find('input').removeAttr('disabled');
				$('#loanStatusModal').find('button[type="submit"]').show();
			})
		});
		function setEdit(id,type=''){
			var action = '{{route('loan-status.show','')}}/'+id
			$.ajax({
				type: 'GET',
				url: action,
				data: {},
				...
				success: function (data) {
					setFormValues('LoanStatus_form',data.inputs);
					$('#loanStatusModal').modal('show');
					if(type=='view'){
						setTimeout(function(){
							$('#loanStatusModal').find('input').attr('disabled','disabled');
							$('#loanStatusModal').find('button[type="submit"]').hide();
						},100);
					}
				},
