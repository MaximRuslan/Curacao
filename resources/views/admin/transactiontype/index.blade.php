...
<div class="row">
	<div class="col-sm-12">
		{{--<div class="btn-group pull-right m-b-20">--}}
			{{--<a data-toggle="modal" href="#transactionTypeModal" class="btn btn-default waves-effect waves-light">Add</a>--}}
		{{--</div>--}}
		<h4 class="page-title">Transaction types</h4>
	</div>
</div>
<div class="row">
	<div class="col-12">
...
@include('common.delete_confirm',[
	'modalId'=>'deleteTransactionType',
	'action'=>route('transaction-types.destroy','deleteId'),
	'item'=>'it',
	'callback'=>'showMsg'
	])

	@include('admin.transactiontype.create')
@endsection
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			oTable = $('#type-table').DataTable({
				processing: true,
				serverSide: true,
				ajax: {
					"url": "{{route('transaction-type-data')}}",
					"type": "POST"
				},
				order: [],
			...
			$('#transactionTypeModal').on('hidden.bs.modal', function () {
				$('#transactionTypeModal').find('form')[0].reset();
				$('#transactionTypeModal').find('form').find('input[name="id"]').val('');
				$('#transactionTypeModal').find('input').removeAttr('disabled');
				$('#transactionTypeModal').find('button[type="submit"]').show();
			})
		});
		function setEdit(id,type=''){
			var action = '{{route('transaction-types.show','')}}/'+id
			$.ajax({
				type: 'GET',
				url: action,
				data: {},
				...
				success: function (data) {
					setFormValues('transactionType_form',data.inputs);
					$('#transactionTypeModal').modal('show');
					if(type=='view'){
						setTimeout(function(){							
							$('#transactionTypeModal').find('input').attr('disabled','disabled');
							$('#transactionTypeModal').find('button[type="submit"]').hide();
						},100);
					}
				},
		...
		function SaveTransactionType(form){
