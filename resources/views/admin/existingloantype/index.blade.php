...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <div class="btn-group pull-right m-b-20">
                <a data-toggle="modal" href="#existingLoanTypeModal" class="btn btn-default waves-effect waves-light">Add</a>
            </div>
            <h4 class="page-title">Existing loan type</h4>
        </div>
    </div>
    <div class="row">
        <div class="col-12">
    ...
    @include('common.delete_confirm',[
        'modalId'=>'deleteExistingLoanYype',
        'action'=>route('existing-loan-type.destroy','deleteId'),
        'item'=>'it',
        'callback'=>'showMsg'
        ])

    @include('admin.existingloantype.create')
@endsection
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            oTable = $('#loan-type-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    "url": "{{route('existing-loan-type-data')}}",
                    "type": "POST"
                },
                order: [],
            ...
            $('#existingLoanTypeModal').on('hidden.bs.modal', function () {
                $('#existingLoanTypeModal').find('form')[0].reset();
                $('#existingLoanTypeModal').find('form').find('input[name="id"]').val('');
                $('#existingLoanTypeModal').find('input').removeAttr('disabled');
                $('#existingLoanTypeModal').find('button[type="submit"]').show();
            })
        });

        function setEdit(id, type='') {
            var action = '{{route('existing-loan-type.show','')}}/' + id
            $.ajax({
                type: 'GET',
                url: action,
                data: {},
                ...
                success: function (data) {
                    setFormValues('existing_loan_type_form', data.inputs);
                    $('#existingLoanTypeModal').modal('show');
                    if (type == 'view') {
                        setTimeout(function () {
                            $('#existingLoanTypeModal').find('input').attr('disabled', 'disabled');
                            $('#existingLoanTypeModal').find('button[type="submit"]').hide();
                        }, 100);
                    }
                },
        ...
        function SaveExistingLoantype(form) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    successMsg('Message');
                    $('#existingLoanTypeModal').modal('hide');
                    oTable.draw(true);
                },
                error: function (jqXHR, exception) {
