var messages = {
    ...
    init() {
        var _this = this;
        _this.validationForm(adminAjaxURL + 'messages', 'post');
    ...
    bindUiActions() {
        var _this = this;
        _this.data.datatable = $('#message-datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                "url": adminAjaxURL + 'datatable-messages',
            },
            columns: [
                {data: 'id', name: 'messages.id', visible: false},
                {data: 'user_name', name: 'users.lastname'},
                {data: 'user_name', name: 'users.firstname', visible: false},
                {data: 'message', name: 'messages.message'},
                {data: 'status', name: 'messages.status'},
                {data: 'created_at', name: 'messages.created_at', searchable: false},
            ],
            order: [[0, 'desc']],
