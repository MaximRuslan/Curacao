var loanReason = {
    el: {
        form: '#loan_reason_form',
        addButton: ".addReason",
        editButton: ".editReason",
        deleteButton: ".deleteReason",
        modal: '#loanReasonsModal',
        deleteLoanReasonModal: '#deleteLoanReasonModal',
        confirmDeleteLoanReasonButton: '.confirmDeleteLoanReasonButton',
    },
    data: {
        datatable: '',
    },
    init() {
        var _this = this;
        _this.validationForm(adminAjaxURL + 'loan-reasons', 'post');
        _this.bindUiActions();
    },
    bindUiActions() {
        var _this = this;
        _this.data.datatable = $('#datatable').DataTable({
            ...
            serverSide: true,
            ajax: {
                "url": adminAjaxURL + 'datatable-loan-reasons',
            },
            columns: [
        ...
        $(document).on('click', _this.el.deleteButton, function (e) {
            e.preventDefault();
            $(_this.el.deleteLoanReasonModal).find(_this.el.confirmDeleteLoanReasonButton).data('id', $(this).data('id'));
            $(_this.el.deleteLoanReasonModal).modal('show');
        });

        $(document).on('click', _this.el.deleteLoanReasonModal + ' ' + _this.el.confirmDeleteLoanReasonButton, function (e) {
            e.preventDefault();
            $.ajax({
                dataType: 'json',
                method: 'delete',
                url: adminAjaxURL + 'loan-reasons/' + $(this).data('id'),
                success: function (data) {
                    _this.data.datatable.draw();
                    $(_this.el.deleteLoanReasonModal).modal('hide');
                }
            })
        });
    ...
    setEdit(id, type) {
        ...
        $.ajax({
            dataType: 'json',
            method: 'get',
            url: adminAjaxURL + 'loan-reasons/' + id + '/edit',
            success(data) {
