...
class LoanDeclineReasonController extends Controller
{
    ...
    {
        return view('admin1.pages.loanDeclineReason.index');
    }

    public function edit(LoanDeclineReason $reason)
    {
        $data = [];
    ...
    {
        $this->validate(request(), LoanDeclineReason::validationRules());
        $id = request('id');
        $reason = LoanDeclineReason::find($id);
        if ($reason) {
            $reason->update(request()->all());
        } else {
            $reason = LoanDeclineReason::create(request()->all());
        }
        $data = [];
        $data['status'] = true;
        return $data;
    ...
    public function destroy(LoanDeclineReason $reason)
    {
        $data = [];
    ...
    {
        $loanStatus = LoanDeclineReason::select('*');
